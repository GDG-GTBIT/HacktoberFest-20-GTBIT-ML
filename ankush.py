...
def Food_Collision():
    if Snake_Head.distance(
            Snake_Food) < 15:
        Snake_Food.goto(random.randint(-470, 470), random.randint(-299, 229))
...
def Border_Collision():
    ...
    if Snake_Head.xcor() > 470 or Snake_Head.xcor() < -470 or Snake_Head.ycor() > 229 or Snake_Head.ycor() < -299:
        time.sleep(1)
        Snake_Head.goto(0, 0)
        Snake_Head.direction = 'stop'
